...
class server_window(QMainWindow):
	def __init__(self, flags, task_queue, channel, key):
		super().__init__()
		# Set app icon
	...
	def user_input_handler(self):
		...
		if user_input == '#clear':
		...
		elif len(user_input) > 6 and user_input[0:5] == '#disc':
			discUser = user_input[6:]
			message = {
				'Code' : 'Disc',
				'User' : 'Admin',
				'discUser' : discUser,
				'Message' : discUser + ' has been disconnected!'
			}
			message = json.dumps(message)
			self.channel.basic_publish(
				exchange = 'group_chat_manager', 
			...
			try:
				...
				# Send it to only expected user:
				user_queue = self.users.get(rec, 'NULL')
	...
	def process_chats(self):
		while not self.task_queue.empty():
			try:
				...
				if code == 'Login':
					queue = data.get('Queue', 'NULL')
					self.users[sender] = queue
				elif code == 'Logout':
					self.users[sender] = 'NULL'
				elif code == 'Alias':
					newU = data.get('New', 'NULL')
					self.users[newU] = self.users[sender]
					self.users[sender] = 'NULL'
...
class init_gui(server_window):
	def __init__(self, flags, task_queue, channel, key):
		...
		server_app = server_window(flags, task_queue, channel, key)
		server_app.show()
