...
class ProfileWidget(QWidget):
	...
	def makeHeader(self):
		try:
			...
			# Prestige Icon
			prestigeIcon = prestigeWidget(self.userObject.getPrestige())

			# User Alias label
			userAliasLabel = QLabel(self.userObject.getAlias())
			userAliasLabel.setObjectName('userAlias')
			userAliasWidget = self.getComboWidget(prestigeIcon, userAliasLabel)

			# User Motto label
			self.userMottoLabel = QLabel(self.userObject.getMotto()[:100])
			...
			repContent = QLabel(self.userObject.getRep())
			...
			memberSinceContent = QLabel(self.userObject.getMemberSince())
			...
			avatarImageName = self.userObject.getAvatar()
			...
			try:
				userFrame = self.userObject.getAvatarFrame()
	...
	def makeBadges(self):
		try:
			...
			badgeCount = 0
			for badgeName, badgeDesc in self.userObject.getBadges().items():
				newBadge = QLabel()
				try:
	...
	def makeAboutMe(self):
		try:
			...
			self.aboutMeContent = QLabel(self.userObject.getAboutMe())
	...
	def makeComments(self, viewer):
		try:
			...
			comments = self.userObject.getComments()	
			...
			self.addCommentEditor.setObjectName('messageField')
